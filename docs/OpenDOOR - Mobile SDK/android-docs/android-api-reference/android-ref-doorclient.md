...
SDK 2.1 provides a coroutine-first modern API, setup-sync visibility during
unlock, explicit unlock cancellation, and improved proximity unlock range.
...
If the lock needs setup sync before unlock, [UnlockEvent.SetupSync](doc:android-ref-unlockevent) is
...
[UnlockFailureReason.LockNotRecognized](doc:android-ref-unlockfailurereason).
...
Cancels an in-flight explicit unlock or proximity unlock attempt and emits
[UnlockEvent.UnlockCanceled](doc:android-ref-unlockevent).
...
SDK 2.1 increases this proximity trigger range compared with earlier
SDK 2.0 builds.
...
Apps should handle [UnlockEvent.SetupSync](doc:android-ref-unlockevent) to show progress when first-time
setup sync is required before a door can unlock, and
[UnlockEvent.UnlockCanceled](doc:android-ref-unlockevent) to react to `cancelUnlock()`.
